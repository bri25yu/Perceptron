...
def load_data(path, symbols):
    ...
    with multiprocessing.Pool(processes=10) as pool:
        results = pool.starmap(load_data_helper, args)
    data = {}
...
def load_data_helper(path, symbol):
    """
...
def weights_to_image(weights, DIM):
    """
    ...
    for row in image_vals:
        image_row = []
        for col in row:
            image_row.append(value_to_pixel(col))
        image.append(image_row)
    return np.array(image)


def shuffle(data, ratio=0.9):
    """
...
def test_accuracy(weights, testing_set_i, testing_set_j):
    """
...
def train_weights(weights, training_set_i, training_set_j):
    """
...
def convert_to_important(weights):
...
def get_test_pixel_indices(data, num):
    randIndices = np.random.choice(
...
def image_to_data(image_path, view):
    """
